...
**状态：11 条全部已修**（见 §6 修复记录，含每条的 commit 与验证方式）。
下表「状态」列的 ✅ 表示已修复并有回归测试；所有修复都经**独立复现**确认
（不是照抄本报告的结论 —— 报告给的诊断有 1 条我实测后调整了结论）。
...
1. **D7 的"5 个不可达"要分成两类。** 报告把 `limit_up` / `limit_down`
   也计入"永不出现"，但它们虽无 `pattern` 产出点，却是 `KIND_FALLBACK`
   的兜底名 —— 没有 `pattern` 的涨跌停告警会退回它们，因此**可达**。
   真正死的是 5 个：`seal` / `break` / `touch`（旧命名，无产出点也不在兜底表）
   + `limit_down_touch` / `open_limit_down`（跌停侧被短路）。
   修完后看板计数从虚报的 **11** 变成真实的 **8**。
2. **D10 的严重度被低估了。** 报告写「严重度低（是悬停提示，不是主行）」，
   但它同时指出这是唯一能看到 `amount` 原始元值的地方 ——
   与同屏表格的「成交额(亿)」差 8 个数量级。同一屏两套单位属于会让人
   算错量的缺陷，实测确认后按"必须修"处理。
...
11 条全部已修，分 6 个 commit（均在 `main` 上）。每条都遵守同一个流程：
**先独立复现 → 再改 → 加回归测试 → 回退修复确认测试报红 → 恢复**。

| commit | 内容 |
|---|---|
| `670808e` | D1 标题印错量 |
| `48fc300` | D11 死键 + 检查器漏检 |
| `dde3c34` | D6 / D7 / D2 |
| `ddb708d` | D5（文案 + metrics 两处） |
| `0033d4e` | D3 / D4 |
| `ef75d58` | D8 / D9 / D10 |

测试基线：**1047 passed / 0 failed / 0 error / 0 skipped**（junit xml 计数）。
另有 8 个 `tools/check_*.py` 与 `dash_render_check.js` 全部 exit 0。

### 修复时几个值得记下来的判断

**D7 选了"补上跌停侧"而不是"删掉这两个信号"。**
跌停侧的机械结构其实**已经 90% 建好了** —— `_make_touch` 会用
`limit_down_touch` 与「触及跌停」文案、`_seal_amount_wan` 会取卖一量、
`_make_seal` 两侧都支持；只有 `_check_side` 在跌停侧直接 `return None`。
既然只差这一步，补齐比删掉更有价值（"打开跌停"对做短线的人是有用信号：
封单被撬开说明有资金接）。而 `seal`/`break`/`touch` 三个旧命名确实没有
产出点也不在兜底表里，**删掉**并在原地留注释写明去向。

**D11 选了"真实现兼容"而不是"删掉旧键"。**
`rebound_off_low_pct` 已经在 `settings.yaml` 里存在，直接删会让照着它
配过的用户**静默**失去设置。实现别名时踩到一个坑并已写进注释：
`DEFAULTS` 里新旧名**都有**（都是 2.0），若用 merged 判断"用户设了旧名没有"
会永远为真，反而把用户设的新名压死 —— 所以单独存了一份只含用户显式传入
键的 `self._user`。四种组合（只新/只旧/都设/都不设）都验过。

**D1 的修复只动标题，不动 detail。**
标题改用 `change`（与分级同源），`window_pct` 仍留在 detail 第一行的
「窗口 N分钟 xx%」—— 那里标签与数值口径本来是一致的。
所以信息没有减少，只是把标题换成了正确的量。

**D3 的 `to_limit_pct` 保留键名但语义变了**（从"距涨停"变成"距限价"，
方向由 `Alert.kind` 决定）。没有额外加 label 键 —— 那只是把 kind 抄一遍。

**D5 在 metrics 里也改了名。** 不只改文案：旧代码把同一个数同时写进
`metrics["seal_amount_wan"]`，而这个键在悬停白名单里，看板会照原样展示，
等于明确告诉用户"这是封单"。故炸板路径改产 `bid1_amount_wan`，
真封板路径的 `seal_amount_wan` 不动（那里措辞是对的）。

### 写测试时踩到的两个坑（都已改进）

1. **D10 的用例第一版是假的。** 它在测试里**抄了一份**白名单键列表，
   于是我往 `spirit.py` 白名单塞一个未映射键来验证时，它照样通过（退出码 0）。
   改成从 `spirit.py` 源码解析白名单后才真抓到。
   教训：测试里抄一份被测数据结构，等于把"防漂移"的用例变成"验证抄得对不对"；
   凡是要防止两处不同步的用例，都必须从其中一处**解析**出清单。
2. **D3 的"取不到限价"用例跑不到。** 我按"构造脏行情喂 `evaluate()`"写，
   实测拿到 0 条告警才发现：能算出 0 限价的只有 `prev_close <= 0`，
   而 `Quote.is_suspended` 恰好把这种行情判为停牌并跳过。
   改为直接调 `_make_alert` 验渲染逻辑，并在 docstring 里写明
   **为什么走不到、为什么仍要保留** —— 免得后人以为用例写错了。

### 顺带修好的一个检查器盲区

`tools/check_config_consumed.py` 第一版只挖掉 `config.py` 的 `DEFAULTS`
声明点，**漏了六个规则模块各自的 `DEFAULTS`**。D11 那个键恰好只在
`spirit_price.DEFAULTS` 里声明，于是躲过了检查。现在挖掉所有模块的
`DEFAULTS`，重跑立刻报出它。
这个盲区比"没有检查"更危险：**看起来在查、其实查不到**。

### 环境坑（两次假失败，都不是代码问题）

本轮两次出现性能用例假失败（`spirit_order` 的 200ms、`limit_board` 的 218ms）。
两次都是**上一批失败的子 agent 留下僵尸 python 进程**，CPU 打到 100%：
其中一次有 7 个进程、两个各占 2.5GB。清掉后分别回到 110ms / 71 项全绿。

- `spirit_order`：在**未改动的 HEAD** 上重跑 10 次，10 次全挂 —— 与改动无关。
- 清干净后同一基准：min 109.6ms / median 128.0ms / max 148.4ms。

**结论：这类失败先看 CPU 负载，别改代码。** 也说明这两条性能断言的余量偏紧
（负载高时 best 会从 110ms 飘到 200ms+），留给后续决定是否放宽或用相对基准。
