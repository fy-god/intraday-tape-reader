...
**最新独立审计**：[`2026-09-20_18-22-15_JST.md`](./2026-09-20_18-22-15_JST.md)（审计线，针对冻结 `f6a064b`）  
最新产品实现轮：[`2026-09-20_17-22-44_JST.md`](./2026-09-20_17-22-44_JST.md)  
...
## 审计线本轮（2026-09-20 18:22 JST，针对冻结 `f6a064b`）

- `reviewed_source_sha` = `f6a064bc787b1bea5e8edfa521b9575a8047d47f`（冻结 worktree）。
- **审计期间检测到并发写入者**，故全部结论建立在冻结快照上，不读工作树。
  审计结束时远端 `main` 已前进到 `100e06a`（自述修 TIME-ROLE-001/002/003 + LIMIT-001）——
  **本审计不为其背书**，那是下一轮的回归对象。
- 冻结树实跑：`1260 passed in 89.26s` exit 0；`tools/check_*.py` 8/8 exit 0；
  `node tools/dash_render_check.js` exit 0。
- **本条最关键**：`IT-P1-COMPLETE-001` 的 `shortfall == 0` 判据被证明不仅"口径不纯"，
  而且产生**永久性功能后果**——市场含停牌股（实测约 6%，`docs/NOTES_tencent.md:100`
  的 358/5908）时 `complete` 恒为 False，股票池刷新永久走 partial 分支，
  被"不得小于现有池"保护拒绝，**新股永不入池（实测 0/10）**。
  已证明是 `c7d4efd` 新引入（`020f2bb` 同场景判 `True`）。
  → 应升级为**已确认错误的 P1**，且不属"口径洁癖"，而是与 `IT-P1-006` 同类危害。
- 另独立复现 9 条（`TIME-ROLE-001/002/003`、`OBS-008/009`、`INDEX-CURRENT-001`、
  `WINDOW-001`、`LIMIT-001`）+ 确认 `OBS-007` 已修正确。
- 详见 [`2026-09-20_18-22-15_JST.md`](./2026-09-20_18-22-15_JST.md)。
...
- [2026-09-20 18:22 JST](2026-09-20_18-22-15_JST.md) — 审计线（冻结 `f6a064b`）；COMPLETE-001 判据升级为 P1 并给出"新股永不入池"实证
- [2026-09-20 17:22 JST](2026-09-20_17-22-44_JST.md) — 产品实现；source-epoch/time-role 时间合同 + LIMIT-001
