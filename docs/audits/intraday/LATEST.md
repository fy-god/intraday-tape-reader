...
**最新云端独立审计（补遗四）**：[2026-09-23_22-21-41_JST.md](./2026-09-23_22-21-41_JST.md)  
**补遗三**：[2026-09-23_22-08-57_JST.md](./2026-09-23_22-08-57_JST.md)  
...
**本轮审计 docs HEAD**：82cdc3ec39c100cd49ef85c278db0d90a1cc4e01  

  > **补遗四（2026-09-23 22:21:41 JST）：本轮最重要的一份 —— 该缺陷类测试结构性看不见。**
  >
  > **新确认（P1，测试有效性）IT-P2-WP01-AXIS-SUITE-BLINDNESS-005**：
  > 在**真 git clone** 里把两个轴都改成**名称盲**（即一个**错误**的修法），
  > **全量测试依然全绿**：
  > `	ext
  > M6（错误修法）         rc=0   1891 passed in 156.33s   ← 错解无人发现
  > 阳性对照（:479 退回修前） rc=1   1 failed, 1890 passed     ← 证明装置确实接在该行为上
  >   FAILED test_wp01_red2_explicit_prefix_is_not_index_identity
  > `
  > 含义：**现有测试无法区分“修对了”与“改错了”**。
  > 新增的 7 条 WP01 测试只钉住 sh600000 那一个形态，
  > 对“名称唯一定身份”的**整类**符号没有任何断言。
  > M6 还暴露第二个未检测破坏：账本键与 Quote.code 错位
  > （sh000922 账本=['000922'] 而 Quote.code=[]）。
  >
  > **再次自我更正：受影响符号 = 7（补遗二写 6）**，并给出闭式刻画：
  > ```text
  > 回归集  ≡  { sym : looks_like_index(sym, 真名, code) != looks_like_index(sym, "", code) }
  > 实测两集合逐元素相同 = True
  > 回归 7: sh000922 中证红利 / sh000015 红利 / sh000903 中证100 /
  >        sh000904 中证200 / sh000906 中证800 / sh000009 上证380 / sh000133 上证150
  > 对照 6 均安全: sh000001/sz399001/sh000300/sh000016/sh000688/sz399006
  > 被修好 2: sh600000 / sz000001
  > ```
  >
  > **判据实为三处**（我此前写“两轴”）：`tencent.py:333`（Quote.code，真名）、
  > :448（账本键，真名）、:479（请求键，**空串**）。
  > 补丁只对齐了判据**形状**，未对齐**输入**。
  >
  > **验收要求**（比前几份更严）：正确修法必须**同时**满足
  > ①名称唯一定身份的指数无假 missing/unexpected；
  > ② _key 与 parse_response:334 的 out_code **逐字节一致**；
  > ③ 两轴判据**输入相同**；并加一条**变异测试**（改成空名必须变红）。
  >
  > **WP03 门控：**在上述变红能力就位前，任何针对该缺陷类的“修复”
  > 都**无法被本仓库的测试验证**。
  >
  > 本轮**未改任何源码**（审计仓库 src 指纹前后一致）；**模型真实增益 = 0**。
