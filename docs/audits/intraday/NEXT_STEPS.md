# NEXT_STEPS — 2026-09-22 01:00 JST

> 排序原则：**先解锁一批缺陷的公共堵点**，再做单点修复。
> 每条都标了"为什么现在做这个"和"什么算做完"。
>
> 🔴 **01:00 轮结论（最重要的一句）**：
> **我 21:00 轮修的"假绿"没有被消除 —— 它从数据层搬到了判决层。**
> 云端用真实实验证明、我独立复现：注入
> `accounting_status=inconsistent / errors=7 / total=0 / named=66` 后，
> `evaluate_health` 仍返回 `healthy=True / exit_code=0`，**10 个判决项与
> 健康基线逐字节相同** —— 因为判决项名单里**没有一项读交付账本**。
> **修一个可观测性缺陷，必须一路走到"有人会因此改变结论"为止；
> 停在"字段有了"就是没修。**
>
> **✅ 01:00 轮修复 3 条**
> | ID | 复核 | 修复 |
> |---|---|---|
> | `IT-H20-DELIVERY-ACCOUNTING-GATE-MISSING` | **成立** | `evaluate_health` 真读交付账本 |
> | `IT-P1-UNIVERSE-WATCHLIST-PIN-001` | **成立** | 降级不再 pin（临时降级不再变永久）|
> | `IT-P1-ALERT-TOTAL-KIND-TRUNCATION-001` | **成立（我自己发现）** | 按 kind 的 total 改用累计 `by_kind` |
>
> **1694 passed**（+18）/ 8 gate 全绿 / selftest 68-6-8-8 /
> 回退验牙 **7 条行为级 RED / 0 结构性**。
>
> **⬇ 本轮两处下调**
> 1. 21:00 "假绿已修" → **下调**：只修到数据层，判决层不读。
>    真正的修复是 01:00 轮。
> 2. 21:00 `RINGBUFFER-001` "已修" → **下调**：同根因的
>    `web.py:603` 当时没查到，01:00 轮才补。**同一个 bug 类漏了第二个实例。**
>
> 🔴 **`R-21`（01:00 新增，下一轮最高优先）**：
> 01:00 轮只给 **delivery** 加了判决项，**universe 仍然没有** ——
> **扫描范围从 5913 掉到 4100（缺 30%）时，`healthy` 照样是 `True` / `exit 0`。**
> 这与本轮修好的是**同一类假绿**，只是对象不同。**我已经知道它存在，不假装没有。**
>
...
### C. `IT-P2-UNIVERSE-STATUS-CACHE-001`：`status.universe` 不是 active universe
- **现状**：`AlertStore.status()` 用 `len(state.quotes)`，而 `state.quotes` 是
  **所有最近准入过的 latest cache**（个股 + 指数都写，且 `EngineState.prune()`
  **不删** `state.quotes`）。所以 `status.universe != len(engine._codes)`。
- **危害**：这个字段最多只能叫 `latest_quote_cache_size`，
  **不能**拿去做 Universe Coverage Gate —— 它会**高估**覆盖。
- **我 21:00 轮未复核**（云端 20:10 新确认）。列为第三优先。

### D0. 🔴 **`R-21`：给 universe 加判决项（01:00 新增，最高优先）**
- **现状（确定性，代码可查）**：01:00 轮给 **delivery** 加了判决项
  （`live_session.py` 的 `add("delivery_accounting", ...)`），
  但判决项名单仍是
  `rounds / fetch / data / coverage / capability / api / sse / memory /
  browser / alerts / delivery_accounting` —— **没有 universe 项**。
- **危害**：**扫描范围从 5913 掉到 4100（缺 30%）时，`healthy` 照样
  `True` / `exit 0`。** 这与 01:00 轮刚修好的是**同一类假绿**
  （"数据算了但没人读"），只是对象不同。
- **为什么最高优先**：它是我**已经知道存在**的、且危害与刚修的完全相同的一类缺陷；
  放着不修就是选择性修复。而且它直接支撑 `R-12` ——
  没有 universe 判决项，覆盖率再修也没有出口。
- **做法**：把 `_universe_meta`（引擎已算）接到 metrics，
  再加 `add("universe_coverage", ..., level="fail"/"warn")`。
  `not_measured` 判 ok（与 `capability`/`delivery_accounting` 一致）。
- **可证伪**：若真实长跑中 active coverage 恒 ≥ 95%，该项永不触发。
- **验收**：注入 `active=4100 / expected=5913` 的 metrics，
  断言 `healthy=False` 且 `fail` 含 `universe_coverage`。
